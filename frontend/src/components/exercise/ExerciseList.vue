...
<template>
  <section class="card">
    <div class="flex items-baseline justify-between px-5 pt-4 pb-3 border-b border-rule">
      <h2 class="headline text-xl">{{ t('exercises.title') }}</h2>
    ...
    <div v-else-if="entries.length === 0" class="p-8 text-center text-muted">{{ t('exercises.empty') }}</div>
    <div v-else v-for="exercise in entries" :key="exercise.id" class="list-row">
      ...
      <span class="tabular text-sm text-right">{{ exercise.members.length }}</span>
    </div>
...
.list-head, .list-row {
  display: grid;
  grid-template-columns: minmax(0, 1fr) 110px 80px 84px;
  gap: 16px;
  padding: 11px 18px;
  align-items: center;
}
