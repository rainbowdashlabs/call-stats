...
@router.get("")
def get_all(*, session: Session = Depends(get_session), grouped: bool = False,
            include_archived: bool = False) -> list[SubjectUsage] | list[MultiSelectGroup]:
    stmt = select(Subject)
    ...
    return sorted([SubjectUsage(id=s.id, name=s.name, group=s.group, archived=s.archived,
